...
export const StickyMobileCTA: React.FC<StickyMobileCTAProps> = ({ onQuoteClick }) => {
  const handleWhatsApp = (e: React.MouseEvent) => {
    ...
    triggerWhatsAppModal({
      destination: 'Kerala Senior Citizen Tour Packages',
      defaultMessage: 'Hi MyHappyJourney, I am looking for comfortable Kerala Senior Citizen Tour Packages.',
    });
  };
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-2xl p-2 px-3 h-[68px] flex items-center justify-between gap-1.5">
      ...
      <button
        ...
        className="flex-1 h-12 bg-[#25D366] active:bg-[#1fbd58] text-white font-bold text-xs rounded-xl flex flex-col items-center justify-center gap-0.5 active:scale-95 transition-all shadow-sm border-none cursor-pointer"
